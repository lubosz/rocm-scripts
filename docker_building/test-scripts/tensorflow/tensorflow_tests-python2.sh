...
cwd=`pwd`
BASEDIR=$HOME/Desktop/rocm_tests/dnns/tf1.8/tensorflow
...
download_tensorflow_benchmarks()
{
	...
	git git checkout -b may22 ddb23306fdc60fefe620e6ce633bcd645561cb0d
...
run_convolutional_quick_test()
{
    ...
    do
        /usr/bin/python ./tutorials/image/mnist/convolutional.py --self_test > out.txt 2>&1
...
run_tutorials_image_mnist()
{
    ...
    /usr/bin/python ./convolutional.py 2>&1 | tee -a tutorials_image_mnist.txt
...
run_tutorials_image_cifar10()
{
    ...
    /usr/bin/python ./cifar10_train.py --max_steps=5000 2>&1 | tee -a tutorials_image_cifar10.txt
    /usr/bin/python ./cifar10_eval.py --run_once=True 2>&1 | tee -a tutorials_image_cifar10.txt
...
run_resnet_on_cifar10()
{
    ...
    timeout 480s /usr/bin/python ./resnet_main.py --train_data_path=cifar10/data_batch* \
...
run_imagenet_classify()
{
    ...
    /usr/bin/python ./classify_image.py 2>&1 | tee -a imagenet_classify.txt
...
run_tf_cnn_benchmarks()
{
    ...
    for m in $MODELS
    do
        /usr/bin/python scripts/tf_cnn_benchmarks/tf_cnn_benchmarks.py --model=$m --num_gpus=1 --num_batches=2500 --print_training_accuracy=True 2>&1 | tee -a tf_cnn_benchmarks_log.txt
    done
	cp -rf tf_cnn_benchmarks_log.txt $LOGDIR
...
run_soumith_benchmarks()
{
    ...
    do
		...
		if grep -q gfx803 gpu_info.log; then
			sed -i 's|input_dim: 128|input_dim: 8|1' imagenet_winners/googlenet.prototxt
			sed -i 's|input_dim: 64|input_dim: 32|1' imagenet_winners/vgg_a.prototxt
		elif [grep -q gfx900 gpu_info.log]; then
			sed -i 's|input_dim: 128|input_dim: 64|1' imagenet_winners/googlenet.prototxt
		fi	
     /usr/bin/python3 $BDIR/benchmark_${m}.py 2>&1 | tee $BDIR/output_${m}.log
    done
    
    # Get a quick summary
    find $BDIR -name "output*.log" -print -exec grep 'across 100 steps' {} \; -exec echo \;
	cp -rf $BDIR/output*.log $LOGDIR
	
	#Total Avg. Execution time : 30mins
	# Expected "final" output of each log:
	#2018-03-29 04:02:15.456070: Forward-backward across 100 steps, 0.074 +/- 0.007 sec / batch
...
run_slim_lenet
run_slim_cifarnet
#run_soumith_benchmarks ; not supported any more with tf>1.8
run_tf_cnn_benchmarks
