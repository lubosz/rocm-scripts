...
wget -P /etc/yum.repos.d/ http://hydlnxpkg.amd.com/dist/redhat/rhel-7.4/workstation/x86_64/rhel-base.repo

######### Root login #########
# Allow root to log into GUI by commenting out the following lines
sed -i 's|\(^.* pam_succeed_if.so user != root quiet.*$\)|#\1|' /etc/pam.d/gdm
...
wget /root/ http://hydlnxpkg.amd.com/dist/redhat/rhel-7.4/workstation/x86_64/gconf.tgz
tar -zxf gconf.tgz
