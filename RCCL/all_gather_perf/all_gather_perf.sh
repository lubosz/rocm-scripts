...
HSA_FORCE_FINE_GRAIN_PCIE=1 NCCL_DEBUG=INFO ./build/all_gather_perf -b 64 -e 128M -f 4 -g $nGPU -o $i -d $j >> tmp.log 2>&1
grep "bandwidth" tmp.log >> tmp1.log
cat tmp.log >> all_gather_perf.log
while read line
do [ -z "$line" ] && continue ;
bandwidth=${line##* }
done < tmp1.log
...
mv Results.ini all_gather_perf.log $cwd
cd $cwd
