...
static void showUsage(const char* fname)
{
    fprintf(stderr, 
            ...
            "\nDescription\n"
            "    This program uses multiple HIP streams in an attempt at optimizing data\n"
            "    transfers between host and multiple HIP devices using hipMemcpyAsync().\n"
            "\nProgram options\n"
            "  --do=<transfer specs>    transfer specification\n"
            "  --streams=<mode>         stream modes for transfers\n"
            "  --list                   list available HIP devices and quit\n"
            ...
            "  <device>                 HIP device to use for transfer\n"
            ...
            "\nMemory options\n"
            "  mapped                   map host memory into HIP address space\n"
...
static void listDevices()
{
    ...
    for (int i = 0; i < deviceCount; ++i)
    {
        ...
        if (isDeviceValid(i))
        {
            fprintf(stderr, " %2d   %-15s   %02x:%02x.%-3x\n",
                    i, prop.name, prop.pciBusID, prop.pciDeviceID, 
                    prop.major);
        }
    }
    fprintf(stderr, "\n");
